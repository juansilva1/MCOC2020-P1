...
El ultimo día se corrigé un signo y la distancia logra bajar más de 600 metros.

Sin embargo no se logro implementar los terminos tecerales.

### Codigo para tecerales
...
for i in np.linspace(2,7,6):
    print(i)
    for j in np.linspace(1,int(i),int(i)):
        j=int(j)
        suma+=(legendre(j,i,sin_teta)*(C[i,j]*cos(j*atan(y/x))+S[i,j]*sin(j*atan(y/x))))/((r)**(i+1))
